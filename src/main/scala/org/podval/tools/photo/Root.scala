...
final class Root(path: String) extends Directory[Root, Int, Year] with DirectoryOfNumbered[Root, Year, Int, Month]:
  ...
  override protected def getDirectory: File = File(path)
  ...
  def parseDirectory(path: String): Directory[?, ?, ?] = parse(path) match
    case directory: Directory[?, ?, ?] => directory
    case path => throw IllegalArgumentException(s"'$path' is not a directory")
