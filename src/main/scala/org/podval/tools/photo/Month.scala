...
final class Month private(
  val year: Year,
  number: Int
) extends Numbered[Year, Month, Int, Day](
  year,
  number
) with DirectoryOfNumbered[Month, Day, String, Picture]:
  require(number >= 1 && number <= 12)

  override def numberedCompanion: NumberedCompanion[Year, Month, Int, Day] = Month

  override def numberedItemCompanion: NumberedCompanion[Month, Day, String, Picture] = Day
